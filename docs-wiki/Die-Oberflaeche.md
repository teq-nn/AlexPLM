...
- **Zahnrad (Einstellungen)** — öffnet das [Konto](Produkt-teilen#das-konto-app-weite-server-identitat).
...
Details: [Mehrbenutzer & Sync](Mehrbenutzer-und-Sync).
...
[Status-LED](Status-LEDs), den Bausteinnamen, die Hauptdatei mit echtem Pfad, einen
...
Hinweise (blockieren nie). Siehe [Aufgaben & Hinweise](Aufgaben-und-Hinweise).
...
**„Verlauf · Graph"** in den [Graph-Raum](Werkbank-und-Graph-Raum#der-graph-raum-verlauf):
...
[lauten Ausnahme](Mehrbenutzer-und-Sync#die-laute-ausnahme) beim Holen oder
